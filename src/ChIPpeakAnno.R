...
peaks <- toGRanges(paste0(DATA_DIR, 'G4ChipIntersect.bed'), format="BED")
peaks[1:2]
...
write.table(anno_df, file=paste0(DATA_DIR, 'G4ChipIntersect.genes.txt'),
            col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)

uniq_genes_df <- unique(anno_df['symbol'])
write.table(uniq_genes_df, file=paste0(DATA_DIR, 'G4ChipIntersect.genes_uniq.txt'),
